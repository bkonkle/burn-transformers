...
// NOTE: Modified from the built-in function to work within an already-async context, and add extra
// detail to errors
pub async fn download_hf_model(model_name: &str) -> (PathBuf, PathBuf) {
    let api = tokio::Api::new().unwrap();
    let repo = api.model(model_name.to_string());

    let model_filepath = repo.get("model.safetensors").await.unwrap_or_else(|e| {
        panic!(
    ...
    let config_filepath = repo.get("config.json").await.unwrap_or_else(|e| {
        panic!(
